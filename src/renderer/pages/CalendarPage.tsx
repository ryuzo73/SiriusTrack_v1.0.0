                ...
                {totalItems > 0 && (
                  <div className="space-y-1">
                    ...
                    <div className="space-y-1 max-h-20 overflow-y-auto">
                      ...
                      {milestones.slice(0, 2).map((milestone, idx) => (
                        <div
                          ...
                          className={`text-xs truncate font-medium ${
                            milestone.status === 'completed' ? 'line-through text-green-400' : 'text-blue-600'
                          }`}
                        >
                          {milestone.title}
                        </div>
                      ...
                      {/* Todoを表示 */}
                      {todos.slice(0, Math.max(0, 4 - milestones.length)).map((todo, idx) => (
                        <div
                          key={`todo-${idx}`}
                          className={`text-xs truncate ${
                            todo.completed ? 'line-through text-apple-gray-400' : 'text-apple-gray-700'
                          }`}
                        >
                          {todo.type === 'weekly' ? '📅' : '📝'} {todo.title}
                        </div>
                      ))}
                      {totalItems > 4 && (
